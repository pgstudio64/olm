...
class TestReassignCornerDoors:
    """Tests pour _reassign_corner_doors (gap-based face correction)."""

    def _make_door(self, face, jh, jf, width=None):
        w = abs(jf - jh) if width is None else width
        return {
            "face": face,
            "offset_px": min(jh, jf),
            "width_px": w,
    ...
    def test_reassign_east_to_south(self):
        """Porte east au coin SE, gap sur south → reassign to south."""
        from olm.ingestion.comb_detection import _reassign_corner_doors
        d = self._make_door("east", 180, 200)
        rect = (0, 0, 200, 200)
        # Gap on south near x=200 (SE corner), no gap on east near y=200
        wall_gaps = {
            "north": [], "south": [(185, 200, 0.1)],
            "east": [], "west": [],
        }
    ...
    def test_reassign_north_to_west_NW(self):
        """Porte north au coin NW, gap sur west → reassign to west."""
        from olm.ingestion.comb_detection import _reassign_corner_doors
        d = self._make_door("north", 0, 20)
        rect = (0, 0, 200, 200)
        wall_gaps = {
            "north": [], "south": [],
            "east": [], "west": [(0, 20, 0.1)],
    ...
    def test_no_reassign_door_not_at_corner(self):
        """Porte east au milieu de la face → pas de reassign."""
        from olm.ingestion.comb_detection import _reassign_corner_doors
        d = self._make_door("east", 80, 120)
        rect = (0, 0, 200, 200)
        wall_gaps = {
            "north": [], "south": [(80, 120, 0.1)],
            "east": [], "west": [],
        }
        result = _reassign_corner_doors([d], rect, wall_gaps, 20)
        assert result[0]["face"] == "east"
